...
- **Saving right after a merge no longer fails with "the binary content … could not be located".** If a merge brought in a PDF, audio, video, or image and autosave kicked in at the same moment you clicked Save & Close, the two saves could trip over each other and one would refuse to save (even though the file did end up merged correctly). This also closes a rare case where a failed save could lose a newly imported file's content.
